---
title: "A1_P1_Student"
author: "Céline Carlsen and Kristian Isen"
date: "29/8/2017"
...
setwd("~/Desktop/ExpMeth3_Assignments/Assignment1_KristianIsen")
```
...
# Extract only digits from VISIT-column
...
```{r}
# Remove punctums from all ID-columns
LU_data$ID = gsub('[[:punct:] ]+', '', LU_data$ID)
...
DATA_demo = select(demo_data, ID, VISIT, Gender, Age, Ethnicity, Diagnosis, ADOS, MullenRaw, ExpressiveLangRaw)
DATA_LU = select(LU_data, ID, VISIT, MOT_MLU, MOT_LUstd, CHI_MLU, CHI_LUstd)
DATA_token = select(token_data, ID, VISIT, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
...
```{r}
# Merging the data in two steps 
DATA_LU_demo = merge(DATA_LU, DATA_demo, by = c("ID", "VISIT"))
DATA = merge(DATA_LU_demo, DATA_token, by = c("ID", "VISIT"))
...
```{r}
# Subsetting a new data frame with select variables of only first visit
d_1 = subset(DATA, VISIT == 1, select = c(ID,  ADOS, nonVerbalIQ, VerbalIQ))

# Renaming the columns
d_1 = plyr::rename(d_1, c("ADOS" = "ADOS1", "nonVerbalIQ" = "nonVerbalIQ1", "VerbalIQ" = "VerbalIQ1"))

# Merging to original dataset
DATA = merge(DATA, d_1, by = c("ID"))

# Deleting original rows
DATA = subset(DATA, select = -c(ADOS, nonVerbalIQ, VerbalIQ))

# Renaming the new rows to old names
DATA = plyr::rename(DATA, c("ADOS1" = "ADOS", "nonVerbalIQ1" = "nonVerbalIQ", "VerbalIQ1" = "VerbalIQ"))

# Reordering the dataset, so it looks better
DATA = DATA[, c("ID", "VISIT", "Gender", "Age", "Ethnicity", "Diagnosis", "ADOS", "nonVerbalIQ", "VerbalIQ", "MOT_MLU", "MOT_LUstd", "CHI_MLU", "CHI_LUstd", "types_MOT", "types_CHI", "tokens_MOT", "tokens_CHI")]
...
# Gender from numbers to F & M
DATA$Gender = as.factor(DATA$Gender)
DATA$Gender = plyr::revalue(DATA$Gender, c("1" = "F", "2" = "M"))
...
```{r}
# Write into CSV
write.csv(DATA, file = "ExpMeth3_Assignment1.csv")
```
...
# 1
ChiMLU_means = by(DATA$CHI_MLU, DATA$ID, FUN = mean)
ChiMLU_means[ChiMLU_means > 2.7]

# 1 -- using filter() -- 
DATA %>% 
  group_by(ID) %>% 
  summarise(ChiMLU_m = mean(CHI_MLU, na.rm = TRUE)) %>% 
  filter(ChiMLU_m > 2.7)

# 2
#filter(DATA, CHI_MLU < 1.5 & VISIT == 1)


# 3
Kids_notFinished = na.omit(DATA) %>% 
      group_by(ID) %>% 
      tally() %>% 
      filter(n < 6)
Kids_notFinished
...
?arrange()
as.numeric(DATA$tokens_CHI)
dplyr::arrange(DATA$VISIT == 6, DATA$tokens_CHI)
...
```{r}
# 1
DATA2 = subset(DATA[DATA$Diagnosis == "ASD"], DATA$CHI_MLU, DATA$tokens_CHI)
...
3. The solution to task above enables us to assess the average amount of words produced by each child. Why don't we just use these average values to describe the language production of the children? What is the advantage of keeping all the data?e

```{r}
by(DATA$CHI_MLU, DATA$ID, FUN = mean)
?mutate()
?aggregate()

DATA$ChiMLU_mean %>%
  group_by(ID) %>% 
  summarise(ChiMLU_m = mean(CHI_MLU))
